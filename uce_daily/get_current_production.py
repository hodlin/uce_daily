...
class OperativeProduction:
            ...
            try:
                ...
                for device in response.json():
                    if device["device_name"][:8] == "Inverter":
                        inverters.append(device["device_id"])
                    self.inverters.update({site_id: tuple(inverters)})
                response.raise_for_status()
            except Exception as err:
                print(f'Exception raised: {err}')
            else:
    ...
    def get_data(self, w_code, date):
        ...
        try:
            data = self.prepare_site_data(site_id, utc_time_index, self.inverters[site_id], "Active power, kW")
            return data
